...
ByteBuffer.prototype.putRawString = function (index, str) {
  if (typeof index === 'string') {
    ...
    // one char => max bytes is 6
    var maxSize = str.length * 6;
    if (maxSize > ONE_HUNDRED_MB) {
      maxSize = Math.ceil(Buffer.byteLength(str) * 1.5);
    }
    this._checkSize(this._offset + maxSize);
  }
